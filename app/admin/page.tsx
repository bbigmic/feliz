    ...
    <div className="min-h-screen bg-darkbg text-darktext">
        ...
        {activeTab === 'users' && (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-darktext">Użytkownicy</h2>
              <button className="btn-primary flex items-center space-x-2" onClick={() => setIsAddUserModalOpen(true)}>
                <Plus className="w-4 h-4" />
                ...
                <div className="flex flex-col gap-4 sm:hidden">
                  {users.map(user => (
                    <div key={user.id} className="bg-darkbg rounded-xl shadow-lg p-4 border border-gray-800">
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          user.role === 'admin' ? 'bg-red-900 text-red-300 border border-red-800' :
                          user.role === 'management' ? 'bg-purple-900 text-purple-300 border border-purple-800' :
                          user.role === 'seller' ? 'bg-blue-900 text-blue-300 border border-blue-800' :
                          'bg-gray-900 text-gray-300 border border-gray-800'
                        }`}>
                          {user.role === 'admin' ? 'Administrator' :
                           user.role === 'management' ? 'Zarządzanie' :
                           user.role === 'seller' ? 'Sprzedaż' : 'Użytkownik'}
                        </span>
                        <button
                          onClick={() => { setEditUser(user); setIsEditUserModalOpen(true) }}
      ...
      {isAddUserModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <motion.div
            ...
            className="bg-darkpanel rounded-lg p-6 w-full max-w-xl mx-4 border border-gray-800"
          >
            <h3 className="text-lg font-semibold mb-4 text-darktext">Dodaj nowego użytkownika</h3>
            <form className="space-y-4" onSubmit={async (e) => {
              e.preventDefault();
              const form = e.target as any;
              const newUser = {
                ...
                marketingAccepted: form.marketingAccepted.checked
              };
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  toast.error(data.error || 'Błąd podczas dodawania użytkownika');
                }
              } catch (error) {
              ...
              <div>
                <label className="block text-sm font-medium text-darksubtle mb-1">Hasło</label>
                <input 
                  name="password" 
                  type="password" 
                  required 
              ...
              <div>
                <label className="block text-sm font-medium text-darksubtle mb-1">Rola</label>
                <select 
                  name="role" 
                  required 
                  className="w-full px-3 py-2 border border-gray-700 rounded-lg bg-darkbg text-darktext"
                >
                  ...
                  <option value="seller">Sprzedaż</option>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2">
                  <input 
                    name="termsAccepted" 
                    type="checkbox" 
                    className="rounded border-gray-700 bg-darkbg text-primary-600 focus:ring-primary-500"
                  />
                  <span className="text-sm text-darksubtle">Akceptacja regulaminu</span>
                </label>
                <label className="flex items-center gap-2">
                  <input 
                    ...
                    className="rounded border-gray-700 bg-darkbg text-primary-600 focus:ring-primary-500"
                  />
                  <span className="text-sm text-darksubtle">Zgoda marketingowa</span>
                </label>
              </div>
              <div className="flex justify-end space-x-3 pt-4">
                <button 
                ...
                <button type="submit" className="btn-primary">Dodaj użytkownika</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {isEditUserModalOpen && editUser && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          ...
          >
            <h3 className="text-lg font-semibold mb-4 text-darktext">Edytuj rolę użytkownika</h3>
            <div className="mb-4 p-3 bg-darkbg rounded-lg border border-gray-700">
              <p className="text-sm text-darksubtle">Email:</p>
              <p className="font-medium text-darktext">{editUser.email}</p>
            </div>
            <form className="space-y-4" onSubmit={async (e) => {
              e.preventDefault();
              const form = e.target as any;
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  toast.error(data.error || 'Błąd podczas aktualizacji roli');
                }
              } catch (error) {
