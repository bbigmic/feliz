-- =====================================================
-- KOMPLETNY BACKUP BAZY DANYCH FELIZTRADE
-- Data utworzenia: $19 sierpnia 2025
-- Wersja: 2.0 (zaktualizowana)
-- 
-- Zawiera:
-- - Wszystkie tabele z najnowszymi polami
-- - Wszystkie indeksy i klucze obce
-- - Wszystkie dane (13 oprogramowań, 14 komponentów, 1 użytkownik, 2 zamówienia, 51 zdjęć)
...
DROP TABLE IF EXISTS "OrderFile" CASCADE;
...
CREATE TABLE "Order" (
    ...
    "phone" TEXT NOT NULL,
    "info" TEXT,
    "orderType" TEXT NOT NULL DEFAULT 'collaboration',
    "status" TEXT NOT NULL DEFAULT 'pending',
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "stripeSessionId" TEXT,
    "termsAccepted" BOOLEAN NOT NULL DEFAULT false,
    "marketingAccepted" BOOLEAN NOT NULL DEFAULT false,
    "collaborationConsentAccepted" BOOLEAN NOT NULL DEFAULT false,
    "codeConsentAccepted" BOOLEAN NOT NULL DEFAULT false,
...
ALTER TABLE "OrderFile" ADD CONSTRAINT "OrderFile_orderId_fkey" 
...
(132, 'https://res.cloudinary.com/dd0dqviwc/image/upload/v1753623721/ixyy8dl2vxtgjiuxstrx.png', false, 5),
...
-- =====================================================
-- DANE ZAMÓWIEŃ (2 zamówienia z zaktualizowanymi polami)
-- =====================================================

INSERT INTO "Order" ("id", "userId", "productId", "email", "phone", "info", "orderType", "status", "createdAt", "stripeSessionId", "termsAccepted", "marketingAccepted", "collaborationConsentAccepted", "codeConsentAccepted", "selectedCategory", "language") VALUES
(2, 3, 2, 'admin@feliztradeltd.com', '657835354', NULL, 'collaboration', 'paid', '2025-07-29T23:31:11.199Z', 'cs_test_a14wDIyRkTyvcOJauyOOm6UMUwrGuyvKH2srUm9ViUjH50QVn1ZgZZmk5M', true, false, true, false, NULL, 'en'),
(1, 3, 8, 'admin@feliztradeltd.com', '996574445', NULL, 'collaboration', 'paid', '2025-07-29T23:27:30.851Z', 'cs_test_a1dF7IMxGtThIXjaYZsoY8jJvXS8fcMAPeb9c3ODtOH92w5rnXxj2Y5WY3', true, false, true, false, NULL, 'en');
...
SELECT setval('"OrderFile_id_seq"', (SELECT MAX(id) FROM "OrderFile"), true);
...
/*
BACKUP ZOSTAŁ UTWORZONY Z:
- 1 użytkownikiem (admin@feliztradeltd.com)
- 13 oprogramowaniami z tłumaczeniami angielskimi
- 51 zdjęciami oprogramowania
- 2 zamówieniami (zaktualizowanymi do nowej struktury)
- 14 komponentami z tłumaczeniami angielskimi
- Wszystkimi najnowszymi polami i tabelami

STRUKTURA BAZY:
- User: 7 pól
- Software: 15 pól (z tłumaczeniami)
- SoftwareImage: 4 pola
- Order: 16 pól (zaktualizowane)
- OrderFile: 7 pól (nowa tabela)
- Component: 7 pól (z tłumaczeniami)

INDEKSY I KLUCZE:
- Unikalny indeks na email użytkownika
- Klucze obce dla wszystkich relacji
- Wszystkie sekwencje ID zresetowane

Aby przywrócić bazę:
1. Uruchom ten plik SQL na czystej bazie PostgreSQL
2. Wszystkie dane zostaną przywrócone
3. Struktura będzie identyczna z oryginalną
*/
